...
program
  ...
  .option('--monthly <n>', `Total monthly capacity BEFORE housing (default: ${d.monthlyInvestment})`)
  .option('--net-monthly <n>', 'Monthly amount you actually invest (auto-adds rent to get gross capacity)')
  .option('--rent <n>', `Monthly rent until flat purchase (default: ${d.monthlyRent})`)
  .option('--rent-start-month <n>', `Month offset when rent begins (default: ${d.rentStartMonth})`)
  .option('--keep-portfolio', 'Do not use portfolio for property purchase (fund via parent loan)')
  .option('--parent-loan-years <n>', `Years to repay parent loan (default: ${d.parentLoanYears})`)
  .option('--cash-rate <n>', `Cash savings interest rate % (default: ${(d.cashRate! * 100).toFixed(1)})`)
...
program
  ...
  .option('--md-output <path>', 'Output markdown summary file path (default: alongside CSV)')
